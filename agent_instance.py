...
async def initialize_plan_agent(model, tools=[]):
    ...
    3. 日常助手团队支持文件操作、代码搜索、终端命令执行、网络搜索等功能。但是他们不负责写代码！日常助手团队可以帮助你分析项目，编写分析文档。日常助手团队不能执行网页浏览和操作的行为，不能使用浏览器！
    4. 代码开发团队只负责写代码和测试代码，不负责文件操作、代码搜索、执行终端、网络搜索等功能。不要让代码开发团队干除了写代码和测试代码以外的任何事情！代码开发团队不能执行网页浏览和操作的行为，不能使用浏览器！
    ...
    9. 由于语义搜索非常消耗时间，建议计划中列出需要语义搜索的文件夹，谨慎在项目根目录下进行语义搜索！
    10. 你返回的计划必须以JSON格式输出，包含steps字段，steps字段是一个数组，数组中每个元素是一个字符串，表示一个步骤。
    """
    if not model:
...
async def initialize_replan_agent(model, tools=[]):
    ...
    15. 由于语义搜索非常消耗时间，建议计划中列出需要语义搜索的文件夹，谨慎在项目根目录下进行语义搜索！
    16. 指定任务时，必须将团队名称作为前缀，比如“日常助手团队: ”和“代码开发团队: ”
    17. 并行数根据任务实际需求为主，可以是2,3,4,...
    18. 计划请以JSON格式输出，包含action字段，action字段应该包含steps字段，steps字段类型List[str]！
    19. 答案请以JSON格式输出，包含action字段，action字段应该包含response字段，response字段类型str！
    """
...
async def initialize_normal_execute_agent(model, tools=[]):
    ...
    5. 删除文件、修改文件、写入文件是不可逆的操作！必须谨慎使用！
...
async def initialize_code_agent(model, tools=[]):
    ...
    在开发测试脚本时：
    1. 不允许修改待测试代码所在的文件
    2. 允许根据待测试代码的算法实现一个测试脚本，用于测试待测试代码的正确性
    3. 测试前端页面的组件必须使用vitest框架
    3. 测试python代码必须使用pytest框架
    4. 测试javascript代码必须使用jest框架
    5. 每个功能的测试用例控制在3个以内
    6. 同一个功能的测试用例必须集中放在一个测试文件中。
    7. 不允许同一个功能的测试用例分散在多个测试文件中。
    8. 测试脚本必须放在test文件夹中。如果test文件夹不存在，必须创建test文件夹。
    ...
    5. 除了测试用脚本，你必须谨慎删除、修改任何文件、写入任何文件！因为这些操作都是不可逆的！
