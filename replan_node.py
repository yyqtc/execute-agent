...
async def replan_node(state: PlanExecute) -> PlanExecute:
    ...
    if result is None:
        ...
        plan_table = [{"desc": re.match(r'\[PARALLEL-(\d+)\]\s*(.+)', step).group(2), "status": "pending"} for step in result.steps]
        todo_write(True, plan_table, state["index"])

        steps = parse_parallel_tasks(result.steps)
        return {"plan": steps, "past_achievement": past_achievement}
